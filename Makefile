...
$(RELEASE)/earth.elf: $(EARTH_DEPS)
	...
	$(RISCV_CC) $(COMMON) earth/earth.s $(filter %.c, $(wildcard $^)) -Tearth/earth.lds -lc -lgcc -o $@
	@$(OBJDUMP) $(OBJDUMP_FLAGS) $@ > $(DEBUG)/earth.lst

$(RELEASE)/grass.elf: $(GRASS_DEPS)
	@echo "$(GREEN)-------- Compile the Grass Layer --------$(END)"
	$(RISCV_CC) $(COMMON) grass/grass.s $(filter %.c, $(wildcard $^)) -Tgrass/grass.lds -lc -lgcc -o $@
	@$(OBJDUMP) $(OBJDUMP_FLAGS) $@ > $(DEBUG)/grass.lst

$(SYSAPP_ELFS): $(RELEASE)/%.elf : apps/system/%.c $(APPS_DEPS)
	@echo "Compile app$(CYAN)" $(patsubst %.c, %, $(notdir $<)) "$(END)=>" $@
	@$(RISCV_CC) $(COMMON) apps/app.s $(filter %.c, $(wildcard $^)) -Tapps/app.lds -lc -lgcc -Iapps -o $@
	@$(OBJDUMP) $(OBJDUMP_FLAGS) $@ > $(patsubst $(RELEASE)/%.elf,$(DEBUG)/%.lst,$@)
...
clean:
	rm -rf build tools/mkfs tools/mkrom tools/disk.img tools/bootROM.bin tools/qemu/qemu.elf
