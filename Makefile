all: apps
	...
	$(RISCV_CC) $(CFLAGS) $(LDFLAGS) $(EARTH_LAYOUT) $(EARTH_SRCS) $(EARTH_LDLIBS) $(INCLUDE) -Iearth/metal -DEARTH -o $(RELEASE_DIR)/earth.elf
...
INCLUDE = -Ilibrary
INCLUDE += -Ilibrary/memory -Ilibrary/file -Ilibrary/syscall -Ilibrary/elf
CFLAGS = -march=rv32imac -mabi=ilp32 -mcmodel=medlow
CFLAGS += -ffunction-sections -fdata-sections

DEFAULT_LDLIBS = -lc -lgcc
LDFLAGS = -Wl,--gc-sections -nostartfiles -nostdlib
EARTH_LDLIBS = -Learth/metal -Wl,--start-group -lc -lgcc -lm -lmetal -lmetal-gloss -Wl,--end-group
